  ...
  const filtered = useMemo(
    () =>
      tools.filter((tool) => {
        ...
        return (
          matchesQuery && (category === "Todas" || tool.category === category)
        );
      }),
    [category, query],
  );
      ...
      <section className="surface-grid relative overflow-hidden rounded-2xl border bg-card px-4 py-8 shadow-sm sm:rounded-3xl sm:px-6 sm:py-10 md:px-10 md:py-14">
        ...
        <div className="relative max-w-3xl space-y-3 sm:space-y-4">
          <Badge variant="secondary" className="gap-1.5">
            <Sparkles className="size-3" /> Workspace operacional
          </Badge>
          <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl md:text-5xl">
            Ferramentas VTEX, em um só lugar.
          </h1>
          <p className="max-w-2xl text-sm text-muted-foreground sm:text-base md:text-lg">
            Acesse utilitários internos para investigar divergências e acelerar
            decisões operacionais com menos troca de contexto.
          </p>
        </div>
      </section>
      ...
      <section className="space-y-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {tools.filter((tool) => tool.enabled).length} ferramenta
              disponível
            </p>
          </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid min-w-0 gap-3 sm:gap-4 md:grid-cols-2 xl:grid-cols-3">
            {filtered.map((tool) => {
              const content = (
                <Card className="group h-full transition duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg">
                  <CardHeader>
                    <div className="mb-3 flex min-w-0 items-start justify-between gap-3">
                      ...
                      >
                        {tool.enabled ? "Disponível" : "Em breve"}
                      </Badge>
                    </div>
                  ...
                  <CardFooter className="border-t bg-muted/20 pt-4 text-sm font-medium">
                    {tool.enabled ? (
                      <>
                        Abrir ferramenta{" "}
                        <ArrowUpRight className="ml-auto size-4 shrink-0 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                      </>
                    ) : (
