...
import {
  getFormattedWeightRange,
  isSameWeightRange,
} from "@/lib/shipping/weightRanges";
...
export function WeightSummary({ weightInfo }: WeightSummaryProps) {
  ...
  const getHeavierLabel = () => {
    ...
    return heavier === "budget1"
      ? "Orçamento 1 mais pesado"
      : "Orçamento 2 mais pesado";
  };
  ...
  return (
    <Card
      title="Peso Total"
      subtitle="Comparação de peso entre os orçamentos (frete CIF-PDO)"
    >
      <div className="grid min-w-0 grid-cols-1 gap-4 md:grid-cols-3 md:gap-6">
        ...
        <div className="text-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
          ...
          <div className="mt-3 px-3 py-2 bg-green-main/10 border border-green-main/20 rounded-md">
            ...
            <p className="text-sm font-semibold text-green-main">
              {budget1Range}
            </p>
          </div>
        </div>
        ...
        <div className="text-center p-4 bg-brand-cyan/5 border border-brand-cyan/20 rounded-lg">
          ...
          <p
            className={`text-xl font-bold sm:text-2xl ${difference !== 0 ? "text-brand-cyan" : "text-gray-600 dark:text-gray-300"}`}
          >
            {difference > 0 ? "+" : ""}
          ...
          {sameRange && (
            <div className="mt-3 px-3 py-2 bg-green-main/10 border border-green-main/20 rounded-md">
              <div className="flex items-center justify-center gap-1.5">
                ...
                <span className="text-xs font-medium text-green-main">
                  Mesma faixa
                </span>
              </div>
            </div>
      ...
      {/* Ícone de peso */}
      <div className="mt-6 flex items-center justify-center gap-2 text-center text-gray-500 dark:text-gray-400">
        <svg
          className="w-5 h-5"
          fill="none"
          viewBox="0 0 24 24"
