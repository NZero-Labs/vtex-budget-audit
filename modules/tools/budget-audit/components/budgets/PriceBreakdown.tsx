  ...
  if (breakdown.length === 0) {
    return (
      <Card
        ...
        subtitle="Por que um orçamento é mais caro que o outro"
      >
        <div className="text-center py-6 text-gray-500 dark:text-gray-400">
          <svg
            className="w-12 h-12 mx-auto mb-3 text-green-main"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
          ...
          <p className="text-sm mt-1">
            Não há diferenças significativas para analisar.
          </p>
        </div>
      </Card>
    );
      ...
      >
        <div className="flex min-w-0 items-start gap-3 sm:items-center">
          ...
          <div className="min-w-0">
            <p
              className={`font-semibold ${cheaperBudget === "equal" ? "text-gray-700 dark:text-gray-200" : "text-green-main"}`}
            >
              {getCheaperText()}
            </p>
            {cheaperBudget !== "equal" && (
              <p className="text-sm text-gray-600 dark:text-gray-300">
                Diferença total: {priceDifference > 0 ? "+" : ""}
