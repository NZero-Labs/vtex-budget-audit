...
- Componente `UserMenu` no header mostrando usuário logado e botão logout
- Script `seed-user.ts` para administradores criarem usuários via CLI
- Novos scripts npm: `db:generate`, `db:push`, `db:studio`, `seed:user`

### Changed

- Layout atualizado com `AuthProvider` e `SessionProvider`
- Rota `/compare` agora é protegida (requer autenticação)
